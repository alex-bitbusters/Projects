...
KiCad PCB project files for the Atari XE Sally adapter are TBD. This `SALLY/PCB/SALLY/` directory is reserved for the future project-specific KiCad files, while the shared reused `65xx.lib` and `65xx.dcm` library files live one level up in `SALLY/PCB/` as the current starting point copied from the MCL64 reference project.
